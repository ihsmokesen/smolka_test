...
public class DynamicProgrammingPacker implements iPacker {
     ...
     * */
    private void restoreSolution(int itemIndex, int capacityIndex, int[][] dynamicMatrix, List<Item> result, List<Item> items) {
        ...
        if (dynamicMatrix[itemIndex-1][capacityIndex] == dynamicMatrix[itemIndex][capacityIndex]) {
            ...
            restoreSolution(itemIndex-1,capacityIndex-items.get(itemIndex-1).getWeight(),dynamicMatrix,result,items);
            if (result == null) {
        ...
        for (int i = 1; i <= items.size(); i++) {
            for (int j = 0; j <= capacity; j++) {
                ...
                if (j - items.get(i-1).getWeight() >= 0) {
                    dynamic[i][j] = Math.max(dynamic[i][j], dynamic[i-1][j-items.get(i-1).getWeight()] + items.get(i-1).getCost());
                }
            }
        }
        return dynamic;
